...
/**
 * Covalent radii for different elements (in Angstroms)
...
export function getAtomRadius(symbol: string): number {
    return (ATOM_RADII[symbol] || 0.7) * VISIBILITY_SCALE;
}

/**
 * Create a sphere geometry for an atom with appropriate radius
 */
export function createAtomGeometry(symbol: string): THREE.SphereGeometry {
    const radius = getAtomRadius(symbol);
    // Use 32 segments for smooth spheres
    return new THREE.SphereGeometry(radius, 32, 32);
}

/**
 * Create a standard material for an atom with CPK color
 */
